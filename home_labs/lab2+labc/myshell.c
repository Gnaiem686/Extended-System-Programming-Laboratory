...
void addProcess(process** process_list, cmdLine* pcmd, pid_t pid){
    ...
    newest_process->cmd = CloneCmdLine(pcmd);
...
void execute(cmdLine *pCmdLine) {
    ...
    if (P_id == -1) {
      ...
      if(pCmdLine->blocking){
        updateProcessStatus(process_list,P_id,TERMINATED);
 ...
 void mypipeline(cmdLine *pCmdLine,cmdLine *NextpCmdLine) {
    ...
    if (child_1 ==-1) {
       ...
       if (pCmdLine->inputRedirect) {
        ...
        if (execvp(pCmdLine->arguments[0],pCmdLine->arguments)==-1) {//creats new prosses
    ...
    if (child_2 ==-1) {
        ...
        if (execvp(NextpCmdLine->arguments[0],NextpCmdLine->arguments)==-1) {//creats new prosses
...
while (1) {
    ...
    if (line[0]== '!'&&line[1]!='!') {
        char aa [1024];
        strncpy(aa, line+1, strlen(line)-1);
        int n = atoi( aa);
